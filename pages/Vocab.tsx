...
import { Plus, Search, Edit2, Trash2, Check, X, Book, RotateCcw, ArrowUpDown, Sparkles, ChevronDown, ChevronUp, Trophy } from 'lucide-react';
      ...
      <div className="animate-soft-in space-y-9">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-2xl font-black text-[#4A4E69] anime-title flex items-center gap-4">
                  <div className="p-2 bg-[#78A2CC]/10 rounded-xl text-[#78A2CC]">
                    <Book size={24} />
                  </div>
                  {STRINGS.vocab.title}
              </h2>
        ...
        <div className="space-y-6">
          <div className="relative group">
              <div className="absolute left-6 top-1/2 -translate-y-1/2 text-[#4A4E69]/20 group-focus-within:text-[#78A2CC] transition-colors">
                  <Search size={20} />
              </div>
          ...
          {filteredData.length > 0 ? (
            <div className="grid grid-cols-1 gap-4">
              {filteredData.map(item => {
                   ...
                   return (
                       <div key={item.id} onClick={(e) => handleEdit(e, item)} className="group bg-white rounded-[28px] border border-[#4A4E69]/5 transition-all cursor-pointer hover:shadow-xl relative overflow-hidden">
                          <div className={clsx("p-5 grid items-center gap-6", GRID_COLS)}>
                              ...
                              <div className="flex items-center justify-center"><div className={clsx("w-full py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest border text-center shadow-sm", getStatusStyle(item.id))}>{stage}</div></div>
                              <div className="flex justify-center gap-2">
                                  <button className="p-2 bg-[#FAF9F6] text-[#4A4E69]/20 hover:text-[#78A2CC] rounded-xl transition-all"><Edit2 size={16} /></button>
                                  <button onClick={(e) => { e.stopPropagation(); if(window.confirm(STRINGS.common.confirmDelete)) deleteVocab([item.id]); }} className="p-2 bg-[#FAF9F6] text-[#4A4E69]/20 hover:text-[#FFB7C5] rounded-xl transition-all"><Trash2 size={16} /></button>
                              </div>
                          </div>
